...
BlastMap analyses any AWS architecture and produces a blast radius audit, including dependency graph, single points of failure, failure cascade simulations, hardening plan, DevOps Agent SKILL.md, and annotated CloudFormation output.
...
description: >
  Pre-computed blast radius intelligence for [app-name]. Use when investigating incidents on [name every resource tagged BlastMapSPOF: true from Step 6 — e.g.
...
─────────────────────────────────────────
CREATE .kiro/hooks/blastmap.yaml
─────────────────────────────────────────
Write this content:

name: BlastMap Blast Radius Analysis
description: Runs blast radius audit on every CloudFormation change
trigger:
  event: file_change
  pattern: "**/*.yaml"
  filter: "CloudFormation"
agent:
  instructions: |
    Run the BlastMap prompt from .kiro/steering/blastmap-prompt.md
    against the changed CloudFormation file.
    Run: git rev-parse --short HEAD to get <commit-hash>.
    Write SKILL.md to .blastmap/SKILL.md
    Write annotated CFN to .blastmap/<commit-hash>-<original-filename>.yaml
...
─────────────────────────────────────────
  ...
  .kiro/steering/blastmap-prompt.md ✓
  .kiro/hooks/blastmap.yaml         ✓
...
4. Select which agent type(s) can use it — `Generic` (default) makes it available to all.
...
Already configured by the setup prompt. The hook in `.kiro/hooks/blastmap.yaml` fires on every CloudFormation save and re-runs the full 6-step audit automatically.
